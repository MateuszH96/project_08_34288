...
void OrderPos::addPos(TYPE_POS typePos) {
    ...
    do {
        ...
        if (input > 0 && input <= menu->getMenuList().size()) {
            ...
            std::cin >> size;
            std::string sizeToAdd(1, toupper(size));
            double priceSize = getPriceBySize(size, input - 1);
            if (priceSize) {
                toAdd = ingredient(sizeToAdd + " - " + menu->getMenuList()[input - 1].getName(), priceSize);
                switch (typePos) {
                    case PIZZA:
...
void OrderPos::addAdditives(std::string file) {
    ...
    do {
        ...
        if (input > 0 && input <= menu->getMenuList().size()) {
            ...
            double priceSize = getPriceBySize(size, input - 1);
            if (priceSize) {
                ingredient toAdd(sizeToAdd + " - " + menu->getMenuList()[input - 1].getName(), priceSize);
                list.push_back(toAdd);
                repeat = false;
            }
        } else if (input == 0) {
